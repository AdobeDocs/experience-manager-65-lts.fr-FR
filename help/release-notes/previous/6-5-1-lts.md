...
* **FORMS-24689:** En mode clé en main, la mise à jour du port de la base de données n&#39;est pas correctement appliquée lors d&#39;une nouvelle installation ou mise à niveau. En mode d’installation récente, les utilisateurs ne peuvent sélectionner que les ports affichés à l’écran. En mode Mise à niveau, le nouveau port de base de données doit être mis à jour manuellement dans lc_turnkey.xml avant de procéder à la mise à niveau.

* **FORMS-24687:** Dans les déploiements de clusters LTS AEM Forms 6.5 sur JBoss EAP 8, les fichiers domain/configuration/domain_oracle.xml, domain_mysql.xml et domain_mssql.xml peuvent contenir une balise `<security>` en double, ce qui entraîne un code XML non valide empêchant le contrôleur de domaine de démarrer.
...
Les utilisateurs et utilisatrices ont subi une corruption du référentiel lors du compactage en ligne si le compactage hors ligne s’était précédemment exécuté sur le référentiel JCR. Un `SegmentNotFoundException` (SNFE) peut se produire dans ce scénario. Cela peut entraîner une corruption du référentiel.

Pour résoudre le problème, installez le correctif via le gestionnaire de packages à partir de [Distribution logicielle](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip). Comme le correctif comprend un lot de `oak-segment-tar` de bas niveau, l’instance redémarre après l’installation.
...
>[!NOTE]
>
> Pour toute opération exécutée par Oak, utilisez le fichier jar [Oak-run 1.78.1-B006](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) (lié ci-dessus).
