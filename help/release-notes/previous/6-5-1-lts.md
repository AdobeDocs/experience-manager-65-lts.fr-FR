...
>[!IMPORTANT]
>
> **Correctif obligatoire** – Pour éviter des problèmes SNFE (SegmentNotFoundException) avec la compression hors ligne lors de l’installation du pack de services 1, installez le correctif décrit dans [Problèmes connus – Corruption du référentiel lors de la compression en ligne](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146).
...
* Oracle Java™ 17/21 (64 bits) (pour plus d’informations, consultez la section [Prise en charge de Java](#java-support).

**Mises à jour du support de serveurs d’applications**
...
AEM Forms sur JBoss® EAP 8 utilise Elytron pour gérer les informations d’identification sécurisées. Les clients doivent configurer une banque d’informations d’identification basé sur Elytron pour réussir le démarrage du serveur et sécuriser l’authentification de la base de données.
...
* Repose sur la conformité Jakarta EE 9
...
* AEM Forms 6.5 LTS SP1 prend en charge **uniquement les serveurs d’applications Jakarta EE 9 et versions ultérieures**
...
À compter ’**AEM Forms 6.5 LTS SP1**, seuls les serveurs d’applications qui implémentent **Jakarta Servlet API 5/6** sont pris en charge. Avec **Jakarta EE 9 et versions ultérieures**, toutes les API sont passées de l’espace de noms `javax.{}` à `jakarta.`.
...
* Mettez à jour toutes les instructions d’importation de `javax.{}` vers `jakarta.`
* Recréez et recompilez les projets DSC personnalisés
...
**Option 2 : migration binaire à l’aide d’Eclipse Transformer**
...
>[!NOTE]
>
> (Forms uniquement) Oracle Java™ 8/11 (64 bits) requis pour une installation unique.
...
**Environment**
S’applique à : AEM 6.5 LTS (On-Premise) avec pack de services 1 (SP1). Le SP1 est fourni sous la forme d’un fichier JAR de démarrage rapide.
...
* **FORMS-24688 :** lors de la configuration de JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) sous Linux, les scripts créés ou modifiés sous Windows peuvent contenir des fins de ligne CRLF, ce qui entraîne l’apparition d’erreurs dans /bin/sh^M: interpréteur incorrect ou $’\r’: commande introuvable. Assurez-vous que les fichiers concernés utilisent les terminaisons de ligne LF. Pour ce faire, utilisez des outils tels que dos2unix ou toute méthode équivalente.

* **FORMS-24689 :** en mode clé en main, la mise à jour du port de la base de données n’est pas correctement appliquée lors d’une nouvelle installation ou mise à niveau. En mode Nouvelle installation, les personnes ne peuvent sélectionner que les ports affichés à l’écran. En mode Mise à niveau, le nouveau port de base de données doit être mis à jour manuellement dans lc_turnkey.xml avant de procéder à la mise à niveau.

* **FORMS-24687 :** dans les déploiements de clusters AEM Forms 6.5 LTS sur JBoss EAP 8, les fichiers domain/configuration/domain_oracle.xml, domain_mysql.xml et domain_mssql.xml peuvent contenir une balise `<security>` en double, ce qui entraîne un code XML non valide empêchant le contrôleur de domaine de démarrer.
...
Les utilisateurs et utilisatrices rencontraient une corruption du référentiel lors du compactage en ligne si le compactage hors ligne s’exécutait précédemment sur le référentiel JCR. Une erreur de type `SegmentNotFoundException` (SNFE) peut se produire dans ce scénario. Cela peut entraîner une corruption du référentiel.

Pour résoudre le problème, installez le correctif via le gestionnaire de packages à partir de la [Distribution logicielle](https://experience.adobe.com/#/downloads/content/software-distribution/fr/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip). Comme le correctif à chaud comprend un bundle `oak-segment-tar` de bas niveau, l’instance redémarre après l’installation.

Planifiez le temps d’arrêt d’une instance lors de son application. Pour le compactage hors ligne, utilisez le fichier jar [Oak-run correspondant](https://experience.adobe.com/#/downloads/content/software-distribution/fr/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar), également disponible sur la distribution logicielle.

>[!NOTE]
>
> Pour toute opération exécutée par Oak, utilisez le fichier jar [Oak-run 1.78.1-B006](https://experience.adobe.com/#/downloads/content/software-distribution/fr/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) (lien ci-dessus).
...
### Lot de `com.adobe.granite.apicontroller` manquant dans AEM 6.5 LTS SP1 (GRANITE-67639) {#missing-apicontroller-bundle-granite-67639}

Le lot `com.adobe.granite.apicontroller` est manquant dans AEM 6.5 LTS SP1. Ce lot contrôle la manière dont les lots OSGi sont résolus et peut empêcher les lots de se résoudre sur d’autres lots, ce qui est utile pour limiter les API exposées.

Installez le correctif à partir de [Distribution logicielle](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-67639-1.0.zip) pour utiliser cette fonctionnalité.
...
* [Liste des offres groupées OSGi incluses dans Experience Manager 6.5 LTS, Pack de services 1](/help/release-notes/assets/65lts_sp1_bundles.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Liste des packages de contenu inclus dans Experience Manager 6.5 LTS, Pack de services 1](/help/release-notes/assets/65lts_sp1_packages.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
