...
* **FORMS-24688:** Lors de la configuration de JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) sous Linux, les scripts modifiés sous Windows peuvent contenir des fins de ligne CRLF, ce qui entraîne l’apparition d’erreurs dans /bin/sh^M : interpréteur incorrect ou $&#39;\r&#39; : commande introuvable. Convertissez les fichiers .sh concernés au format Unix (LF) à l’aide de dos2unix (par exemple, recherchez . -type f -name « *.sh » -exec dos2unix {} +) pour résoudre le problème.

* **FORMS-24689:** En mode de mise à niveau clé en main, les mises à jour du port de base de données ne sont pas sélectionnées dans l&#39;interface utilisateur et doivent être modifiées manuellement dans le fichier de configuration lc_turnkey.xml. S’il n’est pas mis à jour correctement, le système peut continuer à utiliser l’ancienne valeur de port.

* **FORMS-24687:** Dans les déploiements de clusters LTS AEM Forms 6.5.1 sur JBoss EAP 8, `domain/configuration/domain_oracle.xml` peut contenir une balise `<security>` en double, ce qui entraîne un code XML non valide empêchant le démarrage du contrôleur de domaine.
