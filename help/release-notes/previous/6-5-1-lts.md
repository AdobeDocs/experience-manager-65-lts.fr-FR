...
* **FORMS-24688:** lors de la configuration de JBoss EAP 8.0 sous Linux, les scripts shell modifiés sous Windows peuvent contenir des fins de ligne CRLF, ce qui entraîne des erreurs `/bin/sh^M: bad interpreter` ou $`\r` : la commande n’a pas trouvé. Pour résoudre ce problème, convertissez les fichiers concernés en terminaisons de ligne LF à l’aide de dos2unix.

* **FORMS-24689:** En mode de mise à niveau clé en main, la mise à jour du port de base de données dans `lc_turnkey.xml` peut ne pas être appliquée correctement pendant la mise à niveau, ce qui entraîne la référence de l&#39;ancienne valeur de port par le processus.
