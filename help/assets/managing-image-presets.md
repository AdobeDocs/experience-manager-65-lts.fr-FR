...
description: Découvrez les paramètres d’image prédéfinis Dynamic Media, et comment les créer, les modifier et les gérer.
mini-toc-levels: 3
...
## Concepts des paramètres d’image prédéfinis Dynamic Media {#understanding-image-presets}

Tout comme une macro, un paramètre d’image prédéfini est un ensemble prédéfini de commandes de dimensionnement et de formatage enregistrées sous un nom. Pour comprendre le fonctionnement des paramètres d’image prédéfinis, supposons que votre site web exige que chaque image du produit s’affiche dans des tailles, des formats et des taux de compression différents pour être diffusée sur les ordinateurs de bureau et les appareils mobiles.
...
Si vous avez l’intention de prendre en charge l’ingestion de fichiers AI, EPS et PDF de manière à pouvoir générer des rendus dynamiques de ces formats de fichiers, consultez les informations suivantes avant de créer des paramètres d’image prédéfinis.
...
Le dimensionnement des miniatures est défini au format suivant : **`width:height:center`**, par exemple `80:80:false`. La largeur et la hauteur déterminent la taille en pixels de la miniature. La valeur « centrer » est soit false soit true et, si elle est définie sur true, elle indique que l’image miniature a exactement la taille donnée dans la configuration. Si l’image redimensionnée est plus petite, elle est centrée dans la miniature.
...
La création d’un paramètre d’image prédéfini Dynamic Media vous permet d’appliquer ce paramètre à n’importe quelle image lors de la prévisualisation ou de la publication.
...
>[!NOTE]
>
>Pour créer des paramètres d’image prédéfinis Dynamic Media, vous devez disposer de droits d’administration pour Experience Manager ou Admin Console.
...
* **[!UICONTROL Format]** (**[!UICONTROL Onglet De base]**) - Sélectionnez **[!UICONTROL JPEG]** ou un autre format répondant à vos besoins. Tous les navigateurs web prennent en charge le format d’image JPEG. Il offre un bon équilibre entre la petite taille des fichiers et la qualité de l’image. Toutefois, les images au format JPEG utilisent un schéma de compression avec perte qui peut introduire des artefacts d’image indésirables si le paramètre de compression est trop faible. C’est pourquoi Adobe recommande de définir la qualité de compression sur 75. Ce paramètre offre un bon équilibre entre la qualité d’image et la taille de fichier réduite.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>Entrez un nom descriptif sans espaces. Ajoutez la spécification de taille de l’image dans le nom pour permettre l’identification de ce paramètre d’image prédéfini.</td>
  </tr>
  <tr>
   <td><strong>Largeur et hauteur</strong></td>
   <td>Saisissez la taille (en pixels) à laquelle l’image est diffusée. La largeur et la hauteur doivent être supérieures à 0 pixel. Si l’une des valeurs est 0, aucun préréglage n’est créé. Si les deux valeurs sont vides, un paramètre d’image prédéfini réactif est créé.</td>
  </tr>
  <tr>
   ...
   <td><p>Sélectionnez un format dans le menu.</p> <p>La sélection du format <strong>JPEG</strong> présente les options supplémentaires suivantes :</p>
    <ul>
     <li><strong>Qualité</strong> - Contrôle le niveau de compression JPEG. Ce paramètre affecte à la fois la taille du fichier et la qualité de l’image. L’échelle de qualité de JPEG s’étend de 1 à 100. L’échelle est visible lorsque vous faites glisser le curseur.</li>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>Sélectionnez <strong>Aucun</strong>, <strong>Accentuer</strong> ou <strong>Accentuation</strong>. </p>
    ...
    <ul>
     <li><strong>Quantité</strong> : contrôle le degré de contraste appliqué aux pixels de contour. La valeur numérique réelle par défaut est 1,0. Pour les images à haute résolution, vous pouvez l’augmenter jusqu’à 5.0. Considérez la quantité comme une mesure de l’intensité du filtre.</li>
...
* [opac](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/http-protocol-reference/command-reference/r-opac.html?lang=fr#image-serving-api) - Ajuste l’opacité de l’image. Permet de réduire l’opacité du premier plan.
...
## Application de paramètres d’image prédéfinis Dynamic Media {#publishing-image-presets}
...
**Pour publier manuellement les paramètres d’image prédéfinis dans le mode hybride de Dynamic Media, procédez comme suit :**
...
1. Sélectionnez l’icône **[!UICONTROL Outils]**, puis accédez à **[!UICONTROL Ressources]** > **[!UICONTROL Paramètres d’image prédéfinis]**.
