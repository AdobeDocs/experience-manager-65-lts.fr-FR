...
|---|---|---|
| Simplifier l’expérience pour les créatifs utilisant un poste de travail | Simplifiez l’accès aux ressources depuis une gestion des ressources numériques ([!DNL Experience Manager Assets]) pour les créatifs ou, plus largement, pour les utilisateurs sur poste de travail utilisant des applications de création de ressources natives. Ces derniers ont besoin d’une méthode simple et rapide pour découvrir, utiliser (ouvrir), modifier et enregistrer les modifications dans [!DNL Experience Manager], ainsi que pour charger de nouveaux fichiers. | Poste de travail Windows ou Mac ; applications [!DNL Creative Cloud] |
...
|---|---|---|
| Les personnes créatives découvrent des ressources à partir de [!DNL Experience Manager], les ouvrent et les utilisent, les modifient et chargent les modifications dans [!DNL Experience Manager], ou encore chargent les nouveaux fichiers dans [!DNL Experience Manager] sans quitter les applications [!DNL Creative Cloud]. | [Adobe Asset Link](https://helpx.adobe.com/fr/enterprise/using/adobe-asset-link.html) | [!DNL Adobe Photoshop], [!DNL Adobe Illustrator] et [!DNL Adobe InDesign]. |
...
| Téléchargement – plusieurs fichiers/structures de dossiers hiérarchiques | Non | Oui | [Chargement via l’interface web](manage-assets.md#uploading-assets) ou via un outil ou script personnalisé. |
...
Souvent, il n’est pas recommandé de stocker toutes les ressources et les mises à jour. Par exemple, si les mises à jour de ressources spécifiques sont de mauvaise qualité et utilisent les ressources en excès, les ressources peuvent être stockées dans la gestion des ressources numériques (DAM).
...
En règle générale, les équipes créatives internes, de même que les agences et les professionnels de la création intégrés au réseau interne, ont accès au déploiement DAM et peuvent se connecter à [!DNL Experience Manager]. [!DNL Experience Manager] et l’infrastructure réseau peuvent être configurés afin d’autoriser un accès direct aux partenaires externes (généralement, des entreprises de confiance telles que des agences travaillant pour un client ou une cliente) pour disposer d’un accès à [!DNL Experience Manager] via le réseau (par le biais de la liste des adresses IP autorisées ou d’un VPN, par exemple).
...
* Chargement des résultats de séances photo ou de projets plus volumineux
