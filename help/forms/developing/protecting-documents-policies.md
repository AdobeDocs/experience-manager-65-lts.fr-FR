...
1. Définissez les attributs de la politique.
   ...
   * Définissez l’ensemble de politiques auquel appartient la nouvelle politique en appelant la méthode `setPolicySetName` de l’objet `Policy` et en transmettant une valeur de chaîne qui spécifie le nom de l’ensemble de politiques. (Vous pouvez spécifier `null` pour cette valeur de paramètre qui entraîne l’ajout de la politique à l’ensemble de politiques *Mes politiques*.)
...
1. Enregistrez la politique.
   ...
   * Valeur de chaîne représentant l’ensemble de politiques auquel appartient la politique.
   ...
   Si vous utilisez un compte d’administration d’AEM Forms dans les paramètres de connexion pour créer l’objet `DocumentSecurityClient`, alors spécifiez le nom de l’ensemble de politiques lorsque vous appelez la méthode `registerPolicy`. Si vous transmettez une valeur `null` pour l’ensemble de politiques, la politique est créée dans l’ensemble de politiques *Mes politiques* au niveau administration.

   Si vous utilisez un utilisateur Document Security dans les paramètres de connexion, vous pouvez appeler la méthode `registerPolicy` surchargée qui accepte uniquement la politique. En d’autres termes, vous n’avez pas besoin de spécifier le nom de l’ensemble de politiques. Cependant, la politique est ajoutée à l’ensemble de politiques nommé *Mes politiques*. Si vous ne souhaitez pas ajouter la nouvelle politique à cet ensemble de politiques, indiquez un nom d’ensemble de politiques lorsque vous appelez la méthode `registerPolicy`.

   >[!NOTE]
   >
   >Lors de la création d’une politique, référencez un ensemble de politiques existant. Si vous spécifiez un ensemble de politiques qui n’existe pas, une exception est générée.
...
1. Définissez les attributs de la politique.
   ...
   * Définissez l’ensemble de politiques auquel la politique appartient en attribuant une valeur de chaîne au membre de données `policySetName` de l’objet `PolicySpec`. Indiquez un nom d’ensemble de politiques existant. (Vous pouvez spécifier `null` pour cette valeur de paramètre qui entraîne l’ajout de la politique à *Mes politiques*.)
   * Définissez la période de bail hors ligne de la politique en affectant une valeur entière au membre de données `offlineLeasePeriod` de l’objet `PolicySpec`.
...
1. Enregistrez la politique.
   ...
   * Valeur de chaîne représentant l’ensemble de politiques auquel appartient la politique. Vous pouvez définir une valeur `null`, ce qui entraîne l’ajout de la politique à l’ensemble de politiques *Mes politiques*.

   Si vous utilisez un compte d’administration d’AEM Forms dans les paramètres de connexion pour créer l’objet `DocumentSecurityClient`, spécifiez le nom de l’ensemble de politiques lors de l’appel de la méthode `registerPolicy`.

   Si vous utilisez un utilisateur Document Security dans les paramètres de connexion, vous pouvez appeler la méthode de surcharge `registerPolicy` qui accepte uniquement la politique. En d’autres termes, vous n’avez pas besoin de spécifier le nom de l’ensemble de politiques. Cependant, la politique est ajoutée à l’ensemble de politiques nommé *Mes politiques*. Si vous ne souhaitez pas ajouter la nouvelle politique à cet ensemble de politiques, indiquez un nom d’ensemble de politiques lorsque vous appelez la méthode `registerPolicy`.

   >[!NOTE]
   >
   >Lors de la création d’une politique et de la définition d’un ensemble de politiques, assurez-vous de spécifier un ensemble de politiques existant. Si vous spécifiez un ensemble de politiques qui n’existe pas, une exception est générée.
...
Récupérez une politique existante pour la modifier. Pour récupérer une politique, spécifiez le nom de la politique et l’ensemble de politiques auquel elle appartient. Si vous spécifiez une valeur `null` pour le nom de l’ensemble de politiques, la politique est récupérée à partir de l’ensemble de politiques *Mes politiques*.
...
1. Définissez les attributs de la politique.
   ...
   Modifiez les attributs de la politique pour répondre aux besoins de votre entreprise. Par exemple, pour modifier la période de bail hors ligne de la politique, appelez la méthode `setOfflineLeasePeriod` de l’objet `Policy`.
...
Avant d’effectuer une opération de service Document Security par programme, vous devez créer un objet client de service Document Security. Si vous utilisez l’API Java, créez un objet `RightsManagementClient`. Si vous utilisez l’API de service web Document Security, créez un objet `RightsManagementServiceService`.
...
Pour appliquer une politique à un document PDF, référencez une politique existante et indiquez à quel ensemble de politiques elle appartient. L’utilisateur qui définit les propriétés de connexion doit avoir accès à la politique spécifiée. Dans le cas contraire, une exception se produit.
...
1. Enregistrez le formulaire PDF.
   ...
   * Créez un tableau d’octets qui stocke le contenu des données de l’objet `BLOB` qui a été renvoyé par la méthode `protectDocument`. Renseignez le tableau d’octets en obtenant la valeur du membre de données `MTOM` de l’objet `BLOB`.
...
1. Enregistrez le document PDF non sécurisé.
   ...
   * Créez un tableau d’octets qui stocke le contenu des données de l’objet `BLOB` qui a été renvoyé par la méthode `removePolicySecurity`. Renseignez le tableau d’octets en obtenant la valeur du champ `MTOM` de l’objet `BLOB`.
   * Créez un objet `System.IO.BinaryWriter` en utilisant son constructeur et en transmettant l’objet `System.IO.FileStream`.
...
Avant d’effectuer par programmation une opération de service Document Security, créez un objet client de service Document Security. Si vous utilisez l’API Java, créez un objet `RightsManagementClient`. Si vous utilisez l’API de service web Document Security, créez un objet `RightsManagementServiceService`.
...
Si le document est protégé à l’aide d’une politique contenue dans un ensemble de politiques (autre que Mes politiques), alors `RMInspectResult.getPolicysetName` et `RMInspectResult.getPolicysetId` renvoient des chaînes valides.
...
Pour appliquer une politique à un document Word, vous devez référencer une politique existante et spécifier à quel ensemble de politiques elle appartient. L’utilisateur qui définit les propriétés de connexion doit avoir accès à la politique spécifiée. Dans le cas contraire, une exception se produit.
