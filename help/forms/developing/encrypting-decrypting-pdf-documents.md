...
1. Définissez les options d’exécution du chiffrement.
   ...
   * Spécifiez l’option de compatibilité Acrobat en attribuant une valeur d’énumération `PasswordEncryptionCompatibility` au membre de données `compatibility` de l’objet `PasswordEncryptionOptionSpec`. Par exemple, attribuez `PasswordEncryptionCompatibility.ACRO_7` à ce membre de données.
...
Vous devez obtenir un document PDF non chiffré à chiffrer. Si vous tentez de sécuriser un document PDF déjà chiffré, une exception est générée.
...
1. Déverrouillez le document.
   ...
   * Un objet `BLOB` qui contient le document PDF chiffré avec certificat.
   * Une valeur de chaîne qui spécifie le nom d’alias de la clé publique qui correspond à la clé privée utilisée pour chiffrer le document PDF.
